...
```
    ...
    a. To find the number of hospitals reported in this data, we are considering only the unique values for the FAC_NAME variable.

```{python}
print('There are', len(pos2016_filtered['FAC_NAME'].unique()), 'hospitals reported in this data.') # There was no nan in this column
```
